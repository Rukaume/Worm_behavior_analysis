...
from my_functions.functions import rgb_to_gray, sliding_window, get_image_filelist
from my_functions.functions import adjust, preprocessing, curvature_calc, get_ROI
from my_functions.functions import make_directories, make_background_and_subtracted_images
from my_functions.functions import curvature_analysis, image_subtraction_threshold
from my_functions.functions import threshold_GUI, image_subtraction_analysis, image_subtraction_analysis_data_save
from my_functions.functions import lethargus_analyzer
...
def subtraction():
    ...
    datalist = image_subtraction_analysis(imagelist,
                                          ROI_list,
                                          saved_threshold_pixel)
    t2 = time.time()
    elapsed_time = t2 - t1
    print(f"経過時間：{elapsed_time}")
    os.chdir(image_file_path)
    image_subtraction_analysis_data_save(datalist,
                                         image_file_path,
                                         elapsed_time,
                                         date,
                                         threshold)
...
def main():
    subtraction_ans = input("if you already have subtraction data, please put y then, subtraction step is skipped")
